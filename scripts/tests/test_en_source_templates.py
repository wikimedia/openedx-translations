...
Regression cover for the translatewiki.net "empty message group" bug, where an
English .po missing its header fuzzy flag renders as an empty group.
"""
import polib
import pytest
...
# A minimal English source template as xgettext emits it, without the flag.
...
def test_adds_missing_fuzzy_flag(tmp_path):
    """An unmarked English file gets the flag back."""
...
def test_never_touches_translations(tmp_path):
    ...
    assert "nplurals=6" in po.metadata["Plural-Forms"], "Arabic plural rule overwritten"


def test_is_idempotent(tmp_path):
    """A second run is a no-op, so the sync does not churn the diff."""
...
def test_en_check_is_off_by_default(tmp_path):
    """Opt-in per directory, so Atlas passthrough does not fail the build."""
